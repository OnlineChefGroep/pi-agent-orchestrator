...
## 2026-06-06: Custom Agent Loading (I/O Overhead)
*   **Systemic Bottleneck:** `loadFromDir` in `src/custom-agents.ts` was using `Promise.all` and `readFile` from `node:fs/promises` to read potentially thousands of custom agent markdown files. Due to Promise allocation and event-loop microtask scheduling overhead, reading many small local files in an async burst proved slower than simply executing tight, synchronous reads.
*   **Refactor Strategy:** Refactored the `await Promise.all(files.map(async ...))` block into a sequential `for (const file of files)` loop using `readFileSync`.
*   **Key Metric Shift:** Baseline load time for 1,000 custom agent files went from ~2372ms down to ~214ms, a roughly 90% performance gain.
*   **Actionable Principle:** For bulk loading of many small local files, synchronous I/O loops (`readFileSync`) can significantly outperform asynchronous parallel reads (`Promise.all(readFile)`) by avoiding event-loop tick exhaustion and excessive Promise allocations.
