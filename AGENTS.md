...
- **Lint is clean.** `npm run lint` (Biome) passes with no findings. Two findings that previously surfaced on `main` (`test/release-verification.test.ts` `noTemplateCurlyInString`, `test/agent-wizards.test.ts` `organizeImports`) are resolved in this branch.
