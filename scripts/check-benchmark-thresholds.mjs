...
function parseBenchLines(benchLines) {
  ...
  for (const line of benchLines) {
    ...
    const match = line.match(/\[BENCHMARK\] (.+) (.+?)\/(.+?) (\d+)% (.+)/);
    if (!match) {
...
function printResultsTable(results) {
  ...
  console.log(` ${sep}`);
  console.log(` ${"Benchmark".padEnd(nameWidth)}  ${"Measured".padEnd(14)}  ${"Threshold".padEnd(14)}  %     Status`);
  console.log(` ${sep}`);
  ...
  for (const r of results) {
    const statusLabel =
      r.status === "OK" ? color("OK", "OK") : r.status === "WARN" ? color("WARN", "WARN") : color("FAIL", "FAIL");
