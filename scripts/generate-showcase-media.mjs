...
const TMP_DIR = process.env.TMPDIR ? process.env.TMPDIR.replace(/\/$/, "") : os.tmpdir();
...
try {
  ({ getThemeColors, getBoxChars, framedRow, plainTheme } = await import(path.join(root, "dist/ui/theme.js")));
  ({ buildDashboardBodyLines, renderDashboardHeader, renderDashboardFooter, renderDashboardHelp } = await import(
    path.join(root, "dist/ui/dashboard/index.js")
  ));
  ({ getAgentTopEntries, renderTopTable, sortEntries } = await import(
    path.join(root, "dist/ui/agent-top-renderer.js")
  ));
  ...
  const base = [
    {
      id: "agent-explore-1",
      type: "Explore",
      description: "Map src/ and trace RPC handlers",
      status: "running",
      swarmId: "swarm-alpha",
      joinMode: "swarm",
      toolUses: 12 + (frame % 3),
    ...
    },
    {
      id: "agent-explore-2",
      type: "Explore",
      description: "Scan test/ for coverage gaps",
      status: "running",
      swarmId: "swarm-alpha",
      joinMode: "swarm",
      toolUses: 8,
    ...
    },
    {
      id: "agent-plan-1",
      type: "Plan",
      description: "Draft v0.11.0 release checklist",
      status: "running",
      toolUses: 5 + (frame % 2),
      startedAt: t - 20_000,
      spawnedAt: t - 20_000,
    },
    {
      ...
      description: "Implement virtual scroll hints",
      status: "queued",
      toolUses: 0,
      startedAt: t,
      spawnedAt: t,
    },
    {
      id: "agent-gp-2",
      type: "general-purpose",
      description: "Wire swarmHealth RPC audit",
      status: "queued",
      toolUses: 0,
      startedAt: t,
      spawnedAt: t,
    },
    {
      id: "agent-analysis-1",
      type: "Analysis",
      description: "Benchmark fastTruncate paths",
      status: "completed",
      toolUses: 24,
      startedAt: t - 120_000,
    ...
    },
    {
      id: "agent-explore-3",
      type: "Explore",
      description: "Profile dashboard render loop",
      status: "completed",
      toolUses: 18,
      startedAt: t - 90_000,
    ...
    },
    {
      id: "agent-plan-2",
      type: "Plan",
      description: "Security audit schedule bounds",
      status: "error",
      error: "rate limited",
      toolUses: 2,
      startedAt: t - 60_000,
      completedAt: t - 55_000,
      spawnedAt: t - 60_000,
    },
  ];
  return base.map((a, i) => ({
