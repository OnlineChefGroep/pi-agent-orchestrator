 ...
 */
export function getTimeSpinnerFrame(
    now = Date.now(),
...
export class SpinnerEngine {
    ...
    constructor(options: SpinnerOptions = {}) {
        ...
        this.frames =
            options.customFrames ??
            (SPINNER_FRAMES[selectedStyle] as unknown as string[]) ??
    ...
    public getFrame(): string {
        const rawFrame =
            this.frames[this.currentFrameIndex % this.frames.length] ?? "";
    ...
    public toString(): string {
        const pfx =
            typeof this.prefix === "function" ? this.prefix() : this.prefix;
        const sfx =
            typeof this.suffix === "function" ? this.suffix() : this.suffix;
     ...
     */
    public start(
        terminalMode = typeof process !== "undefined" &&
    ...
    public update(
        ...
        if (options.style) {
            this.frames =
                (SPINNER_FRAMES[options.style] as unknown as string[]) ??
                this.frames;
        }
 ...
 */
export function makeDualSpinner(
    style: SpinnerStyle,
    text: string,
): SpinnerOptions {
    return {
...
export function getAlternateFrame(
    ...
    const frames = SPINNER_FRAMES[
        select ? styleA : styleB
    ] as unknown as string[];
    return (
        frames[Math.floor(Date.now() / ANIMATION_INTERVAL) % frames.length] ??
        ""
    );
}
...
export function runShowcase() {
    ...
    const engine = new SpinnerEngine({
        ...
        prefix: "  ",
        suffix: () =>
            `  [Thema: ${keys[currentKeyIndex]}]  - Druk op Ctrl+C om te stoppen`,
    });
