...
export function renderRunningCard(
  ...
  const line1 = wrapBg(` ${th.border}${box.l}${th.reset} ${fastTruncate(`${checked}${icon} ${name}${" ".repeat(gap)}${stats}`, contentW)} ${th.border}${box.r}${th.reset}`, bgSel || bg);
  const line2 = wrapBg(` ${th.border}${box.l}${th.reset} ${fastTruncate(`${th.muted}${description}${th.reset}`, contentW)} ${th.border}${box.r}${th.reset}`, bgSel || bg);
  ...
  if (rec.status === "running" && activity?.maxTurns) {
    ...
    const line3 = wrapBg(` ${th.border}${box.l}${th.reset} ${fastTruncate(progressContent, contentW)} ${th.border}${box.r}${th.reset}`, bgSel || bg);
