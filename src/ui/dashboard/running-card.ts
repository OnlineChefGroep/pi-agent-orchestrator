...
import { padVisible } from "../theme.js";
...
export function renderRunningCard(
  ...
  const line1 = wrapBg(` ${th.border}${box.l}${th.reset} ${truncateToWidth(padVisible(`${checked}${icon} ${name}${" ".repeat(gap)}${stats}`, contentW), contentW)} ${th.border}${box.r}${th.reset}`, bgSel || bg);
  const line2 = wrapBg(` ${th.border}${box.l}${th.reset} ${truncateToWidth(padVisible(`${th.muted}${description}${th.reset}`, contentW), contentW)} ${th.border}${box.r}${th.reset}`, bgSel || bg);
  const bottom = wrapBg(` ${th.border}${box.bl}${box.h.repeat(Math.max(0, cardW - 2))}${box.br}${th.reset}`, bg);
  const actPad = " ".repeat(Math.max(1, 3));
  const actLine = `${actPad}${actIcon} ${th.dim}${truncateToWidth(act, Math.max(8, innerW - 8))}${th.reset}`;
  ...
  if (rec.status === "running" && activity?.maxTurns) {
    ...
    const line3 = wrapBg(` ${th.border}${box.l}${th.reset} ${truncateToWidth(padVisible(progressContent, contentW), contentW)} ${th.border}${box.r}${th.reset}`, bgSel || bg);
