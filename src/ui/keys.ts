/**
 * Terminal key matching mirrored from host `@earendil-works/pi-tui` (0.80.10).
 *
 * The host passes RAW terminal bytes to `Component.handleInput(data)` — never
 * pre-normalised names like "up" / "space" / "enter". This module parses legacy
 * ANSI sequences, Kitty CSI-u, and xterm modifyOtherKeys so dashboard hotkeys
 * work. Do not import `@earendil-works/pi-tui` (AGENTS.md Mistake #4).
 *
 * Also accepts canonical keyIds as `data` (e.g. data === "enter") for tests and
 * defensive compatibility with any host that already normalised input.
 */
...
export const Key = {
    ...
    shiftCtrl: (key: string) => `shift+ctrl+${key}`,
    ctrlAlt: (key: string) => `ctrl+alt+${key}`,
    altCtrl: (key: string) => `alt+ctrl+${key}`,
    shiftAlt: (key: string) => `shift+alt+${key}`,
    altShift: (key: string) => `alt+shift+${key}`,
    ctrlSuper: (key: string) => `ctrl+super+${key}`,
    superCtrl: (key: string) => `super+ctrl+${key}`,
    shiftSuper: (key: string) => `shift+super+${key}`,
    superShift: (key: string) => `super+shift+${key}`,
    altSuper: (key: string) => `alt+super+${key}`,
    superAlt: (key: string) => `super+alt+${key}`,
    // Triple modifiers
    ctrlShiftAlt: (key: string) => `ctrl+shift+alt+${key}`,
    ctrlShiftSuper: (key: string) => `ctrl+shift+super+${key}`,
};
// =============================================================================
// Constants
// =============================================================================
...
const matchesLegacySequence = (data: string, sequences: readonly string[] | undefined): boolean => Boolean(sequences?.includes(data));
const matchesLegacyModifierSequence = (data: string, key: string, modifier: number) => {
    if (modifier === MODIFIERS.shift) {
        return matchesLegacySequence(data, LEGACY_SHIFT_SEQUENCES[key as keyof typeof LEGACY_SHIFT_SEQUENCES]);
    }
    if (modifier === MODIFIERS.ctrl) {
        return matchesLegacySequence(data, LEGACY_CTRL_SEQUENCES[key as keyof typeof LEGACY_CTRL_SEQUENCES]);
    }
    return false;
};
...
function parseKittySequence(data: string): { codepoint: number; shiftedKey?: number; baseLayoutKey?: number; modifier: number; eventType: "press" | "repeat" | "release" } | null {
    ...
    if (arrowMatch) {
        ...
        _lastEventType = eventType;
        return { codepoint: arrowCodes[arrowMatch[3] as keyof typeof arrowCodes], modifier: modValue - 1, eventType };
    }
    ...
    if (funcMatch) {
        ...
        const codepoint = funcCodes[keyNum as keyof typeof funcCodes];
...
export function matchesKey(data: string, keyId: string): boolean {
    // Canonical pass-through for tests / already-normalised hosts.
    ...
    switch (key) {
        ...
        case "f12": {
            ...
            return matchesLegacySequence(data, LEGACY_KEY_SEQUENCES[functionKey as keyof typeof LEGACY_KEY_SEQUENCES]);
        }
    }
...
export function parseKey(data: string): string | undefined {
    ...
    const legacySequenceKeyId = (LEGACY_SEQUENCE_KEY_IDS as Record<string, string>)[data];
