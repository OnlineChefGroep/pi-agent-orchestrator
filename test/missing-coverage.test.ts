...
import { SwarmCoordinator, type SwarmDeliveryCallback } from "../src/swarm-join.js";
...
describe("Missing tests coverage", () => {
  ...
  it("Missing test for formatHealthReport", () => {
    ...
    // Assert the report is actually rendered from the input, not just that the
    // word "health" appears somewhere.
    expect(formatted).toContain(`# /agents health — ${report.timestamp}`);
    expect(formatted).toContain("## Process");
    expect(formatted).toContain("node      : v18");
  ...
  it("Missing test for SwarmCoordinator class", () => {
    // The coordinator's constructor takes a delivery callback, not a config
    // object; swarms are created afterwards via createSwarm(config).
    const deliver: SwarmDeliveryCallback = () => {};
    const coord = new SwarmCoordinator(deliver);
