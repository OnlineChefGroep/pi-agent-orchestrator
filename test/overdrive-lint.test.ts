...
describe("detect-filter-map-join", () => {
	it("flags .filter().map().join() chain", async () => {
		const { detectFilterMapJoin } = await import(
			"../scripts/overdrive/detect-filter-map-join.mjs"
		);
...
describe("detect-double-compute", () => {
	it("flags .trim() called twice on same identifier", async () => {
		const { detectDoubleCompute } = await import(
			"../scripts/overdrive/detect-double-compute.mjs"
		);
...
describe("detect-shift-in-loop", () => {
	it("flags .shift() inside while loop", async () => {
		const { detectShiftInLoop } = await import(
			"../scripts/overdrive/detect-shift-in-loop.mjs"
		);
...
describe("overdrive pattern catalogue", () => {
	it("documents all 8 patterns", async () => {
		...
		const content = readFileSync(
			resolve(__dirname, "../docs/overdrive-patterns.md"),
			"utf-8",
		);
