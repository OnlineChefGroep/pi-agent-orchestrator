...
if (args[0] === "gh" || args[0] === "gh.exe" || args[0].endsWith("gh.exe") || args[0].endsWith("gh")) {
  args.shift();
}
...
${ghHandler("process.argv.slice(process.argv.findIndex(x => x.toLowerCase().endsWith('gh.exe')) > -1 ? process.argv.findIndex(x => x.toLowerCase().endsWith('gh.exe')) + 1 : 1)", statePath)}}
`,
    );
