...
function writeGhStub(binDir: string, statePath: string): string {
  ...
  // its resolved absolute path (e.g. D:\...\release). So the bare "release"
  // token never appears in argv: findIndex(x => x === "release") returns -1 and
  // slice(-1) would drop everything but the last arg. Recover the subcommand
  // from basename(argv[1]) and append the remaining args from slice(2).
